...
class EditBlogPage extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`/api/blog/${this.props.params.id}`).then(res => {
      //todo: handle res code
      return res.json()
    }).then((res) => {
      this.setState({title: res.title, content: res.content, categoryID: res.categoryID})
    }).catch(e => {
      //todo: handle err
      console.log(e)
    })
      ...
      <div className='body'>
        ...
        <div style={{maxWidth: "1920px", minWidth: "755px", margin: "0 auto"}}>
          ...
          <RightBody style={{display: "block", marginLeft: "12%", padding: "30px"}}>
            ...
            <RetroInput style={{margin: "0 auto", width: "100%", fontSize: "15px", display: "block", border: "2px solid lightgreen"}} value={this.state.title} onChange={this.onTitleInputChange} />

            <span style={{fontSize: "20px"}}>Category</span><br />
            <RetroOption data={this.state.categories} style={{width: "50%", fontSize: "15px", display: "block"}} defaultID={this.state.categoryID} onChange={this.onCategorySelectChange}></RetroOption>

            <span style={{fontSize: "20px"}}>Content</span><br />
            <RetroTextArea style={{margin: "0 auto", width: "100%", height: this.state.height + "px", fontSize: "15px", display: "block", minHeight: "300px"}} value={this.state.content} onChange={this.onContentInputChange} />
  ...
  async onConfirmClick() {
    ...
    let res = await fetch(`/api/blog/${this.props.params.id}`, {
      method: "PUT",
      headers: {
