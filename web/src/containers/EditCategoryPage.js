...
class EditCategoryPage extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`/api/category/${this.props.params.id}`).then(res => {
      ...
      <div className='body'>
        ...
        <div style={{maxWidth: "1920px", minWidth: "755px", margin: "0 auto"}}>
          ...
          <RightBody style={{height: this.state.height}}>
            <div style={{fontSize: "30px", width: "100%", textAlign: "center"}}>Edit new category</div>
            <hr style={{border: "1px solid lightgreen"}} />
            {this.state.alertMsg}
            <span style={{fontSize: "20px", marginTop: "20px", display: "inline-block"}}>Name</span><br />
            <RetroInput style={{margin: "0 auto", width: "95%", fontSize: "15px", display: "block", border: "2px solid lightgreen"}} value={this.state.name} onChange={this.onNameInputChange} />

            <span style={{fontSize: "20px", marginTop: "20px", display: "inline-block"}}>Description</span><br />
            <RetroInput style={{margin: "0 auto", width: "95%", fontSize: "15px", display: "block", border: "2px solid lightgreen"}} value={this.state.description} onChange={this.onDescriptionInputChange} />

            <RetroButton style={{fontSize: "20px", margin: "20px auto", display: "block", width: "20%"}} onClick={this.onCreateClick}>Confirm</RetroButton>
          </RightBody>
        </div>
      </div>
    )
  ...
  onCreateClick() {
    ...
    fetch(`/api/category/${this.props.params.id}`, {
      method: "PUT",
      headers: {
